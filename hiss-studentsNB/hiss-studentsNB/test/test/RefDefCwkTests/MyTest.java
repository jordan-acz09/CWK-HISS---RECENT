...
public class MyTest {
    ...
    public void staffCanOnlyRejoinIfOnLeave() {
        ...
        String result = pr.staffRejoinTeam("Amir");
        assertTrue("Staff not on leave should not be able to rejoin.", result.contains("not in team so can't return from holiday"));
        // Simulate Amir going on leave by doing a job they can do (if your logic supports this)
        pr.doJob(1000); // Assume job 1000 can be done by Amir
        String resultAfterLeave = pr.staffRejoinTeam("Amir");
        // Now Amir should be able to rejoin from leave
