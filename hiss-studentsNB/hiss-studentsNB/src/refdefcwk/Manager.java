...
//**********************Jobs************************* 
 ...
 // ***************** Team Staff ************************   
   ...
   /** Returns true if the staff with the specified name 
    ...
    {
    String outcome = "\nJob No " + jbNo;
    Job job = getAJob(jbNo);  // use private helper!
    ...
    Staff staff = getStaffForJob(job); // use private helper!
    ...
    if (staff != null) {
        if (staff.getExperience() >= job.getExperienceRequired()) {
            ...
            // Optionally mark the job as done if your Job class supports it
            // job.setDone(true);
...
{
    allStaff.clear();
    // Staff as per Appendix A with explicit retainer for every member
